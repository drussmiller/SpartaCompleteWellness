      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-2">
              If you paused the program and want to resume where you left off, select the week where you want to re-engage. You will resume on the current day of the week. Missed weeks will move to the end of your program, while your Program Start Date, posts, points, and history are preserved.
            </p>
          </div>
        </CardContent>
      </Card>
...
function DynamicHelpContent({ htmlContent }: { htmlContent: string }) {
  ...
  return (
    <AppLayout>
      ...
      <main className={`pb-24 space-y-4 max-w-2xl mx-auto w-full pl-6 pr-4 py-6 text-lg mt-[40px] md:mt-[100px] ${isAndroid ? 'pb-[calc(24+40)px]' : ''}`}>
        {hasCustomContent ? (
          <DynamicHelpContent htmlContent={pageData!.content!} />
        ) : (
          <DefaultHelpContent />
        )}
      </main>
