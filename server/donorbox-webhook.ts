...
function verifyDonorboxSignature(payload: string, signature: string | undefined, secret: string): boolean {
  if (!signature || !secret) {
    return false;
  }
...
donorboxWebhookRouter.post('/api/webhooks/donorbox', async (req: Request, res: Response) => {
  try {
    ...
    const payload = req.body as DonorboxWebhookPayload;
