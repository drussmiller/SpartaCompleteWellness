...
- **Private Organizations**: Organizations can be marked as private via `isPrivate` boolean flag. Private organizations are hidden from the "Connect with Organization" dropdown on the Join page but remain fully manageable in the Admin Dashboard. Toggle available in both create and edit organization forms. A "Private" badge is shown on organization cards in the admin view.
...
## Recent Changes
- **Apr 6, 2026**: Admin program reset. When an admin changes a user's Program Start Date to a later date (moving them to an earlier week), all posts and media from the corresponding week onward are automatically deleted (excluding introductory videos). Points are recalculated. Logic in `PATCH /api/users/:userId` in `server/routes.ts`, modeled after the re-engage endpoint.
- **Apr 5, 2026**: Comment & reaction notifications. When someone comments on or reacts to a post, the post author receives an in-app notification (with WebSocket real-time delivery). Notifications show comment/reaction icons and are clickable — tapping opens the post's comments view. Closing the comments view returns to the Notifications page (not Home). Reaction notifications are deduplicated (max one per post per hour). Schema: `notifications.post_id` column added. Files: `server/routes.ts` (comment + reaction handlers), `notifications-page.tsx`, `comments-page.tsx`.
- **Apr 5, 2026**: Messaging & Contacts refactor. Messages page now shows conversation history (users you've messaged or received messages from) instead of team members list. Added new Contacts page (`contacts-page.tsx`) accessible via Menu, styled like the Welcome page. Contacts page shows two sections: "Team Members" (auto-populated from user's team) and "Added Contacts" (persisted in `contacts` table). A "+" button opens an Add Contact dialog scoped to same-organization users (admins see all users). Added contacts can be removed with the X button. New schema: `contacts` table (`userId`, `contactUserId`). New API endpoints: `GET /api/contacts`, `POST /api/contacts`, `DELETE /api/contacts/:contactUserId`, `GET /api/messages/conversations`, `GET /api/users/same-organization`.
