...
- November 12, 2025. Simplified SMS notification settings UI - Removed phone number input field and test SMS button from Notification Settings page. Phone number is now managed exclusively through user profile page. SMS toggle in Notification Settings uses phone number from user profile and validates it before allowing SMS to be enabled. Added helper text that directs users to add phone number in profile if not set. This simplifies the UI and creates a single source of truth for phone number management.
- November 13, 2025. Set SMS notifications to opt-in only - Changed SMS notifications to default to OFF for all users. Users must explicitly enable SMS notifications in their Notification Settings. Updated all existing users to have SMS disabled, requiring them to opt in if they want to receive text message notifications. This ensures users have full control over receiving SMS and prevents unwanted text messages.
- November 13, 2025. Added SMS notification disclosure to release waiver - Updated the liability waiver to include section 6 "SMS TEXT MESSAGE NOTIFICATIONS" which informs users that they can opt in or opt out of receiving SMS text messages through their notification settings. Includes standard disclosure about message and data rates and variable message frequency.
```
